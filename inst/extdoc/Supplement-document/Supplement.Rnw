...
<<set-options, include=FALSE, cache=FALSE>>=
  ...
  opts_chunk$set(eval=TRUE,dev="png",dpi=400,tidy=TRUE,message=FALSE,cache=TRUE,warning=FALSE,size="small",fig.width=7.5,fig.height=2.5)
@
...
p1 = plot( select(subset,"TcdA"), xlim=c(-0.01,0.7), points=TRUE, title="TcdA") + ylab("Normalize Impedance")
p2 = plot( select(t.subset,"TcdB"), xlim=c(-0.01,1.2), points=TRUE, title="TcdB") + ylab("Normalize Impedance")
...
<< Figure2, fig.height=4.5, results='hide'>>=
figure_2(wells)
@
...
<<Figure4, fig.height=5>>=
figure_4(wells)
@

\subsection{ Figure S1 }
<<Figure5, fig.height=2.5>>=
...
<<j774-2_txt, fig.width=3.3, fig.height=2.5, out.width="3.3in", out.height="2.5in">>=
