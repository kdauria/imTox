...
The code and package is well documented, describing any smoothing or interpolation performed.
...
p1 = tox_plot( select(subset, "gdTcdB & !(TcdA | TcdB)")) + ggtitle("gdTcdB")
p2 = tox_plot( select(subset, "TcdA[1] | gdTcdB[10] & !TcdB | (TcdA[100] & !gdTcdB)")) + ggtitle("TcdA 1 ng/ml")
