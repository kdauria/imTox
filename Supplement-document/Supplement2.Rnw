...
% command for making a table within the xcell of a table
...
<<set-options, include=FALSE, cache=FALSE>>=
  opts_knit$set(root.dir="../",width=27)
  opts_chunk$set(eval=TRUE,dev="png",dpi=400,tidy=TRUE,message=FALSE,cache=TRUE,
                 tidy.opts=list(width.cutoff=27),out.width="\\columnwidth")
@
...
\section{ Reproducing Figures }\label{S:ReproFig}
...
<< preFigure2, fig.width=7.5, fig.height=4.5>>=
...
<<Figure3top, fig.width=7.5, fig.height=2.5>>=
###### Panel A
subset = retrieveWells(wells, file = c("J774-a.txt", "J774-b.txt"))
t.subset = normalize_toxin(subset, xlim = c(-Inf, 100))
...
<<Figure4, fig.width=7.5, fig.height=5>>=
...
<<Figure5, fig.width=7.5, fig.height=4.5>>=
##### Panel A
subset = retrieveWells(wells, file = "J774-4.txt")
t.subset = normalize_toxin(subset, xlim = c(-2, Inf))
